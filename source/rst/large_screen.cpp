...
public:
  void Init(ui::ScreenContext& ctx) override {
    ...
    ui::GetCommonLayouts().touch_panel =
        ui::LayoutMgr::Instance().MakeLayout("Joker.Main.Play.Hud2");
    MainScreen::Init(ctx);
...
extern "C" {
RST_HOOK void rst_MainScreen_Init(ui::MainScreen&, ui::ScreenContext& ctx) {
  ScopedHookGuard guard;
  if (!guard.AlreadyInHook())
    s_hud.Init(ctx);
}

RST_HOOK void rst_MainScreen_Open(ui::MainScreen&, ui::ScreenContext& ctx) {
  ScopedHookGuard guard;
  if (!guard.AlreadyInHook())
    s_hud.Open(ctx);
}

RST_HOOK void rst_MainScreen_Calc(ui::MainScreen&, ui::ScreenContext& ctx) {
  ScopedHookGuard guard;
  if (!guard.AlreadyInHook())
    s_hud.Calc(ctx);
}

RST_HOOK void rst_MainScreen_Draw(ui::MainScreen&, ui::ScreenContext& ctx) {
  ScopedHookGuard guard;
  if (!guard.AlreadyInHook())
    s_hud.Draw(ctx);
}

RST_HOOK void rst_MainScreen_PrepareClose(ui::MainScreen&, ui::ScreenContext& ctx) {
  ScopedHookGuard guard;
  if (!guard.AlreadyInHook())
    s_hud.PrepareClose(ctx);
}

RST_HOOK void rst_MainScreen_CalcClose(ui::MainScreen&, ui::ScreenContext& ctx) {
  ScopedHookGuard guard;
  if (!guard.AlreadyInHook())
    s_hud.CalcClose(ctx);
}

RST_HOOK void rst_MainScreen_UpdateButtonUsability(ui::MainScreen&, game::HudState& hud_state) {
  s_hud.UpdateButtonUsability(hud_state);
}

RST_HOOK void rst_MainScreen_MagicPlayAndHideYellowGauge(ui::MainScreen&) {
  s_hud.MagicPlayAndHideYellowGauge();
}

RST_HOOK void rst_MainScreen_MagicHideYellowGauge(ui::MainScreen&) {
  s_hud.MagicHideYellowGauge();
}

RST_HOOK void rst_MainScreen_MagicStartUseAnim(ui::MainScreen&) {
  s_hud.MagicStartUseAnim();
}

RST_HOOK void rst_MainScreen_MagicUpdateCost(ui::MainScreen&) {
  s_hud.MagicUpdateCost();
}

RST_HOOK void rst_BActionIconUpdate(ui::PlayBActionIcon& icon, game::GlobalContext* gctx) {
  icon.Update(gctx);
  UpdateBIconForHumanLink(icon, gctx);
}

RST_HOOK void rst_LoadPackages() {
